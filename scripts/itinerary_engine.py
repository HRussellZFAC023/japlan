...
TIME_WINDOWS: Sequence[Tuple[int, str]] = [
    (0, "Overnight (00:00-05:00)"),
    (5, "Early Morning (05:00-08:00)"),
    (8, "Morning (08:00-11:00)"),
    (11, "Midday (11:00-14:00)"),
    (14, "Afternoon (14:00-17:00)"),
    (17, "Evening (17:00-20:00)"),
    (20, "Night (20:00-23:00)"),
    (23, "Late Night (23:00-05:00)"),
]
...
MANUAL_LABEL_MAP: List[Tuple[str, str]] = [
    ...
    ("Torisei Hirakata", "Hirakata Welcome Dinner"),
...
DAY_BLUEPRINTS: List[DayBlueprint] = [
    DayBlueprint(
        day=1,
        title="Arrival & Hirakata Welcome",
        regions=["Kansai Airport", "Osaka", "Hirakata"],
        required_labels=["KIX Arrival", "Agora Riverside Base", "Hirakata Welcome Dinner"],
        optional_labels=[],
        max_clusters=3,
    ...
    DayBlueprint(
        day=2,
        title="Osaka Icons & Minami",
        regions=["Osaka"],
        required_labels=["Osaka Castle Morning", "Osaka Minami Street Food", "Umeda Skylines"],
        optional_labels=["Brooklyn Roasting Break"],
        max_clusters=4,
        start_time=time(9, 0),
        friends=["Henry", "Nanako"],
    ...
    DayBlueprint(
        day=3,
        title="Universal Studios Focus",
        regions=["Osaka"],
        required_labels=["Universal Studios Japan Adventure"],
        optional_labels=[],
        max_clusters=1,
    ...
    DayBlueprint(
        ...
        regions=["Kyoto", "Uji"],
        required_labels=["Kyoto Higashiyama Morning", "Kyoto Downtown Bites", "Pontocho Alley", "Fushimi Sunset Gates", "Fushimi Sake Cellar", "Kyomachiya Stay"],
        optional_labels=[],
        max_clusters=6,
        start_time=time(6, 0),
        friends=["Henry", "Nanako"],
    ...
    DayBlueprint(
        ...
        optional_labels=["Daikanyama Stroll"],
        max_clusters=4,
        start_time=time(9, 0),
    ...
    DayBlueprint(
        ...
        regions=["Nara", "Osaka"],
        required_labels=["Nara Deer Route", "Kuromon Market Crawl", "Kaiyukan Twilight Wander"],
        optional_labels=["Nakatanidou Mochi"],
        max_clusters=4,
        start_time=time(7, 30),
        friends=["Henry", "Nanako"],
    ...
    DayBlueprint(
        ...
        regions=["Kyoto"],
        required_labels=["Philosopher's Path", "Okutan Nanzenji Lunch", "Tōfuku-ji Canopies"],
        optional_labels=["Kyoto Zen Gardens", "Kyoto Zen North"],
        max_clusters=4,
        start_time=time(8, 0),
        friends=["Henry", "Nanako", "Nicole", "Ken"],
...
def duration_for(place: Place) -> timedelta:
    ...
    if name.lower().startswith("tokyo disneysea"):
        minutes = 540
    if "nozomi" in name.lower():
        minutes = 150
    if "kansai international airport" in name.lower():
        minutes = 120
    if name == "Osaka Castle":
        minutes = 120
    if name == "Umeda Sky Building":
        minutes = 90
    if name == "Grand Front Osaka":
        minutes = 90
    if name == "Himeji Castle":
        minutes = 150
    if name == "Kobe Nunobiki Herb Gardens":
        minutes = 120
    if name == "Steakland Kobe":
        minutes = 75
    if name == "Kobe Harborland":
        minutes = 90
    if name == "Café & Meal MUJI":
        minutes = 75
    if name == "Kushikatsu Tanaka":
        minutes = 90
    if name == "Hotel Agora Osaka Moriguchi":
        minutes = 180
    if name == "Kyomachiya Ryokan Sakura Urushitei":
        minutes = 150
    if name == "Arima Onsen Taiko-no-yu":
        minutes = 180
    if name == "Gokurakuyu Hirakata":
        minutes = 150
    if name == "Kiyomizu-dera":
        minutes = 120
    if name == "Sannenzaka & Ninenzaka":
        minutes = 60
    if name == "Nishiki Market":
        minutes = 90
    if name == "Honke Owariya":
        minutes = 75
    if name == "Fushimi Inari Taisha":
        minutes = 150
    if name == "Gekkeikan Okura Sake Museum":
        minutes = 75
    if name == "Hanamikoji Street":
        minutes = 45
    if name == "Kyoto Yakiniku Hiro":
        minutes = 120
    if name == "Arashiyama Bamboo Forest":
        minutes = 90
    if name == "Tenryu-ji":
        minutes = 90
    if name == "Arashiyama Yoshimura":
        minutes = 75
    if name == "Kimono Forest":
        minutes = 45
    if name == "Kinkaku-ji":
        minutes = 75
    if name == "Byōdō-in":
    ...
    if name == "Tsuen Tea":
        minutes = 75
    if name == "Toyosu Market":
        minutes = 120
    if name == "teamLab Planets":
        minutes = 120
    if name == "Pokémon Café":
        minutes = 90
    if name == "Odaiba Seaside Park":
        minutes = 120
    if name == "MIMARU Tokyo Shinjuku WEST":
        minutes = 150
    if name == "Ichiran Ramen":
        minutes = 60
    if name == "Senso-ji":
        minutes = 90
    if name == "Asakusaimahan":
        minutes = 90
    if name == "Tokyo Skytree":
        minutes = 120
    if name == "Tokyo Character Street":
        minutes = 75
    if name == "Tōdai-ji":
        minutes = 120
    if name == "Nara Park":
    ...
    if name == "Nakatanidou":
        minutes = 30
    if name == "Kuromon Market":
        minutes = 90
    if name == "Osaka Aquarium Kaiyukan":
        minutes = 150
    if name == "Tempozan Marketplace":
        minutes = 75
    if name == "Namba to Koyasan":
        minutes = 150
    if name == "Kongobu-ji":
        minutes = 90
    if name == "Eko-in Temple":
        minutes = 240
    if name == "Okunoin":
        minutes = 120
    if name == "W Osaka":
        minutes = 180
    if name == "Afternoon Glam & Restock":
        minutes = 120
    if name == "MYDO Teppanyaki":
        minutes = 150
    if name == "Karaoke Kan Shinsaibashi":
        minutes = 120
    if name == "Bar Nayuta":
        minutes = 90
    if name == "Eikan-dō Zenrin-ji":
        minutes = 90
    if name == "Okutan Nanzenji":
        minutes = 90
    if name == "Tōfuku-ji":
        minutes = 120
    if name == "Ryōan-ji":
        minutes = 90
    if name == "Takamura Wine & Coffee":
        minutes = 90
    if name == "Rinku Premium Outlets":
        minutes = 180
    return timedelta(minutes=minutes)


def describe_time_window(moment: datetime) -> str:
    hour_value = moment.hour + moment.minute / 60.0
    for index, (start_hour, label) in enumerate(TIME_WINDOWS):
        next_start_hour = TIME_WINDOWS[(index + 1) % len(TIME_WINDOWS)][0]
        if start_hour <= next_start_hour:
            if start_hour <= hour_value < next_start_hour:
                return label
        else:
            if hour_value >= start_hour or hour_value < next_start_hour:
                return label
    return "Flexible"
...
def apply_itinerary(rows: List[Dict[str, str]]) -> Tuple[List[Dict[str, str]], List[Cluster], List[DaySummary]]:
    ...
    for place in places:
        ...
        row["Date"] = timestamp.strftime("%B %d, %Y")
        row["Day"] = f"Day {place.day_index}"
        row["Weekday"] = timestamp.strftime("%A")
        row["Time"] = describe_time_window(timestamp)
        ...
        row["_order"] = timestamp.isoformat()
