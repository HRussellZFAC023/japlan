...
let googleMapsSdkCallbackId = 0;

function ensureGoogleMapsSdkLoaded(apiKey, { language, region } = {}) {
  if (typeof window === "undefined" || typeof document === "undefined") {
    throw new Error("Google Maps JavaScript API is not available in this environment.");
  }
  ...
  if (!apiKey) {
    return Promise.reject(
      new Error("Google Directions API key is required for this request.")
    );
  }

  googleMapsSdkPromise = new Promise((resolve, reject) => {
    const callbackName = `__gmapsCallback_${Date.now()}_${++googleMapsSdkCallbackId}`;
...
function resolveTransitModes(transitMode, maps) {
  if (!maps?.TransitMode) {
    return [];
  }
  const modeMap = {
    ...
    FERRY: maps.TransitMode.FERRY,
  };

  const values = Array.isArray(transitMode)
    ? transitMode
...
function applyAvoidanceOptions(request, avoid) {
  if (!Array.isArray(avoid) || !avoid.length) {
    return;
  }
  const avoidSet = new Set(avoid.map((item) => String(item || "").toLowerCase()));
  if (avoidSet.has("highways")) {
    request.avoidHighways = true;
  }
  if (avoidSet.has("tolls")) {
    request.avoidTolls = true;
  }
  if (avoidSet.has("ferries")) {
    request.avoidFerries = true;
  }
  if (avoidSet.has("indoor")) {
    request.avoidIndoor = true;
  }
}
...
async function requestGoogleDirectionsRoute({
  ...
  const maps = await ensureGoogleMapsSdkLoaded(apiKey, { language, region });
  const travelModeKey = String(mode || "driving").toLowerCase();
  const travelMode =
  ...
  if (maps.UnitSystem && typeof maps.UnitSystem.METRIC !== "undefined") {
    request.unitSystem = maps.UnitSystem.METRIC;
  }
  if (region) {
    request.region = region;
  }

  const waypointList = coords.slice(1, -1).map(toLatLngLiteral).filter(Boolean);
  if (waypointList.length) {
    request.waypoints = waypointList.map((location) => ({ location }));
  }

  if (travelMode === maps.TravelMode.TRANSIT) {
    const transitOptions = {};
    const departure = normalizeDepartureDate(
      typeof departureTime === "number" && Number.isFinite(departureTime)
        ? Math.max(departureTime, Date.now())
        : Date.now()
    );
    transitOptions.departureTime = departure;
    const modes = resolveTransitModes(transitMode, maps);
    if (modes.length) {
      transitOptions.modes = modes;
    }
    if (maps.TransitRoutePreference?.FEWER_TRANSFERS) {
      transitOptions.routingPreference = maps.TransitRoutePreference.FEWER_TRANSFERS;
    }
    request.transitOptions = transitOptions;
  } else if (travelMode === maps.TravelMode.DRIVING) {
    if (typeof departureTime === "number" && Number.isFinite(departureTime)) {
      ...
      request.drivingOptions = {
        ...
        trafficModel: maps.TrafficModel ? maps.TrafficModel.BEST_GUESS : undefined,
      };
    }
  }

  applyAvoidanceOptions(request, avoid);
  ...
  const route = await new Promise((resolve, reject) => {
    service.route(request, (result, status) => {
      const okStatus =
        (maps.DirectionsStatus && maps.DirectionsStatus.OK) || "OK";
      if (status === okStatus) {
        const foundRoute =
          result && Array.isArray(result.routes) && result.routes.length
            ? result.routes[0]
            : null;
        if (!foundRoute) {
          reject(new Error("No route found for the selected stops."));
        } else {
          resolve(foundRoute);
        }
        return;
      ...
      const errorMessage =
        ...
        (typeof status === "string" ? status : "Directions request failed.");
